...
// Token de connexion (JWT) : émis par POST /api/auth/login, conservé par le
// navigateur et renvoyé à chaque appel dans l'en-tête Authorization.
...
// Appel API : JSON + token + gestion d'erreur ; renvoie vers la connexion si la
// session a expiré (token périmé, compte supprimé).
async function api(url, options = {}) {
  ...
  if (res.status === 401 && !surLaPageDeConnexion) {
    oublierSession();
  ...
  if (!res.ok) throw new Error(data.error || data.erreur || `Erreur ${res.status}`);
  return data;
}
...
const ICONES = {
  logo: '<path d="M8.5 5v14M8.5 12.5c0-2.2 1.6-3.8 3.7-3.8s3.8 1.6 3.8 3.8V19"/>',   // gardée en secours
...
const STATUTS = {
  admin: { label: 'Administrateur', court: 'Admin',  classe: 'statut-admin', icone: 'couronne', accueil: '/admin.html' },
  modo:  { label: 'Modérateur',     court: 'Modo',   classe: 'statut-modo',  icone: 'bouclier', accueil: '/moderation.html' },
  user:  { label: 'Membre',         court: 'Membre', classe: 'statut-user',  icone: 'profil',   accueil: '/accueil.html' }
};
...
let MOI = null;
let CONFIG = { mediasActifs: false, nomMin: 3, nomMax: 20, mdpMin: 12, mdpMax: 64 };

// Réglages communs serveur / interface (GET /api/config) : une seule vérité.
async function chargerConfig() {
  try { CONFIG = { ...CONFIG, ...(await api('/api/config')) }; } catch { /* valeurs par défaut */ }
  return CONFIG;
}
...
async function demarrerApp(actif) {
  ...
  [MOI] = await Promise.all([api('/api/auth/moi'), chargerConfig()]);
...
async function majCompteurs() {
  ...
  try {
    const [notifs, contacts] = await Promise.all([api('/api/notifications'), api('/api/messagerie/contacts')]);
...
function ouvrirComposeur() {
  ...
  const m = ouvrirModal({
    ...
    corps: `
      ...
      ${CONFIG.mediasActifs ? '' : `<div class="alerte alerte-neutre">${icone('info')}<span>Photo et vidéo arrivent bientôt : l’envoi de médias est désactivé pour le moment, les publications texte fonctionnent normalement.</span></div>`}`,
    pied: `
      <button class="btn btn-soft" data-media="photo" ${CONFIG.mediasActifs ? '' : 'title="Bientôt disponible" disabled'}>${icone('image')}Photo</button>
      <button class="btn btn-soft" data-media="video" ${CONFIG.mediasActifs ? '' : 'title="Bientôt disponible" disabled'}>${icone('camera')}Vidéo</button>
  ...
  const maj = () => {
    ...
    publier.disabled = !texte.value.trim();
  };
  texte.oninput = maj;
  texte.focus();

  // Photo / vidéo : l'envoi de fichier se fait sur la page dédiée
  m.dialog.querySelectorAll('[data-media]').forEach(b => {
    b.onclick = () => { m.fermer(); location.href = `/publier.html#${b.dataset.media}`; };
  });
  ...
  publier.onclick = async () => {
    ...
    try {
      await api('/api/publications', { method: 'POST', body: { description: texte.value.trim() } });
      m.fermer();
...
async function demarrerConsole(actif, rolesAutorises) {
  if (!jetonDeSession()) { location.href = PAGE_CONNEXION; return null; }
  [MOI] = await Promise.all([api('/api/auth/moi'), chargerConfig()]);
