...
function cartePublication(p, { onSupprime = null } = {}) {
  ...
  function brancher() {
    ...
    $('.post-media img')?.addEventListener('click', () => ouvrirVisionneuse(p.media));
    ...
    $('[data-like]').onclick = async () => {
      ...
      try {
        // Côté serveur, aimer retire automatiquement le « je n'aime pas » (et l'inverse).
        await api(`/api/publications/${p.id}/like`, { method: avant.mon_like ? 'DELETE' : 'POST' });
    ...
    $('[data-dislike]').onclick = async () => {
      ...
      rendre();
      try { await api(`/api/publications/${p.id}/dislike`, { method: avant.mon_dislike ? 'DELETE' : 'POST' }); }
      catch (err) { Object.assign(p, avant); rendre(); toastErreur(err); }
    };
...
// Une publication peut porter une photo ou une vidéo (envoi désactivé pour le
// moment, mais l'affichage reste en place pour les contenus déjà en base).
