...
export async function publierPhoto(db, { idUtilisateur, description, nomFichier, typeFichier }) {
  if (!idUtilisateur) {
    throw new ErreurPublication("L'identifiant de l'utilisateur est requis.");
  }
  if (!nomFichier) {
    throw new ErreurPublication('Le fichier image est manquant.');
  }
  if (!typeFichier || !typeFichier.startsWith('image/')) {
    throw new ErreurPublication('Le type de fichier doit être une image.');
  }
