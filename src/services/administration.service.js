 ...
 * suppression de comptes, tableau de bord, journal et fiche d'un
 * compte. Les règles de protection (pas d'action sur soi-même ni
 * sur un autre administrateur) sont appliquées ici, pas dans
 * l'interface.
 * ------------------------------------------------------------
 */
...
import { definirRole } from '../repositories/utilisateur.repository.js';
...
const ROLES_LIBELLES = { user: 'membre', modo: 'modérateur', admin: 'administrateur' };
...
// Nommer un modérateur (ou un administrateur), ou rendre le rôle de membre.
export function changerRole(db, req, id, role) {
  ...
  journaliser(db, req, 'role_modifie', {
    cible: cibleCompte(cible),
    details: `${ROLES_LIBELLES[cible.role] ?? cible.role} → ${ROLES_LIBELLES[role]}`
  });
...
export function suspendre(db, req, id, { raison, jours }) {
  ...
  const duree = jours == null || jours === '' ? null : Number(jours);
  ...
  try {
    administrationRepository.supprimerCompte(db, cible.id, (bilan) => {
      journaliser(db, req, 'compte_supprime', {
        cible: cibleCompte(cible),
        details: `${texte} — supprimés : ${bilan.publications} publication(s), ${bilan.messages} message(s)`
      });
    });
  } catch (err) {
    console.error(err);
...
// Fiche d'un compte : chiffres, volume d'actions par catégorie, actions subies.
