...
// Rattrapage : les publications créées sans passer par l'API (données de départ) n'ont pas
// leurs #hashtags dans la table hashtag — on les extrait une fois pour que les tendances soient justes.
{
  const sansTags = db.prepare(`
  ...
  for (const p of sansTags) {
    const tags = String(p.description).match(/#[\p{L}\p{N}_]+/gu) || [];
    for (const t of new Set(tags.map(t => t.toLowerCase()))) insTag.run(t, p.id);
  }
}
